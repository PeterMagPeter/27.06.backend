...
export function startWebSocketConnection(server: any) {
  ...
  io.on("connection", (socket: any) => {
    ...
    socket.on("sendGetLobbies", async (gameMode: string) => {
      ...
      if (lobbies) io.emit("getLobbies", lobbies); // players, hostname
    });

    socket.on("sendExistRoom", async (roomID: string, username: string) => {
      let lobby = await joinOnlineMatch(roomID, username);
      console.log(lobby, " roomExists???");
      if (lobby !== null) socket.emit("roomExists", roomID);
      else socket.emit("NietRoom", roomID)
    });
    // join a room/ lobby,
    socket.on("sendJoinRoom", async (roomId: string, username: string) => {
      // an db senden wo und wer joined
      let lobby = await joinOnlineMatch(roomId, username);
      console.log("sendJoinRoom ", lobby);
      if (lobby) {
        socket.join(roomId);
    ...
    socket.on(
      ...
      (roomId: string, ships: Ship[], username: string, team: number, rotated: boolean) => {
        // WRITE INTO DB
        io.to(roomId).emit("partnerChangedShips", ships, username, team, rotated);
      }
    );
