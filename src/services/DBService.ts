...
export async function registerUser(registerRes: RegisterResource): Promise<UserResource> {
  ...
  let userInit: UserResource = {
    ...
    const usernameCheck = await client
      .db("OceanCombat")
      .collection("Users")
      .findOne({ username: registerRes.username });
    if (emailCheck !== null) {
      // console.log("Email already registered. Please provide another email to continue!");
      // return null;
      throw new Error(
        "Email already registered. Please provide another email to continue!"
      );
    }
    if (usernameCheck !== null) {
      // console.log("Username already registered. Please provide another username to continue!");
      // return null;
      throw new Error(
        "Username already registered. Please provide another username to continue!"
      );
    }
    ...
    // If everything went fine, return UserResource
    return userInit;
  } catch (error) {
    console.log("registerUser: An error has been thrown.");
    throw error;
  } finally {
    await client.close();
  }
    ...
    if (user !== null) {
      ...
      if (userRes.email !== undefined && userRes.email !== user.email) {
        ...
        if (result !== null) {
          ...
          // console.log("Mail address already in use!");
          // return null;
        } else {
      ...
      if (
        userRes.username !== undefined &&
        userRes.username !== user.username
      ) {
        throw new Error("Username is unique can't be changed!");
        // console.log("Username is unique can't be changed!");
        // return null;
      }
      ...
      if (
        userRes.gameSound !== undefined &&
        userRes.gameSound !== user.gameSound
      ) {
      ...
      if (
        userRes.higherLvlChallenge !== undefined &&
        userRes.higherLvlChallenge !== user.higherLvlChallenge
      ) {
      ...
      if (changedMail) {
        user.verificationTimer = new Date(
          Date.now() + ExpirationTime.TwentyFourHours
        );
        user.verified = false;
...
export async function hostOnlineMatch(
  ...
  try {
    ...
    const pubCollAsArray = await db
      .collection("PublicOnlinematches")
      .find()
      .toArray();
    if (pubCollAsArray.length > 0) {
      const pupOldMatchesFilter = pubCollAsArray.filter(
        (match) =>
          match.createdAt < timeCheck2h && match.gamestatus !== Gamestatus.Test
      );
      if (pupOldMatchesFilter.length > 0) {
        const matchIds = pupOldMatchesFilter.map((match) => match._id);
        await db
          .collection("PublicOnlinematches")
          .deleteMany({ _id: { $in: matchIds } });
      }
    }
    // Check all private matches
    const privCollAsArray = await db
      .collection("PrivateOnlinematches")
      .find()
      .toArray();
    if (privCollAsArray.length > 0) {
      const privOldMatchesFilter = privCollAsArray.filter(
        (match) =>
          match.createdAt < timeCheck2h && match.gamestatus !== Gamestatus.Test
      );
    ...
    const collectionName = onlineMatch.privateMatch
      ? "PrivateOnlinematches"
      : "PublicOnlinematches";
...
export async function joinOnlineMatch(
  roomId: string,
  username: string
): Promise<OnlineMatchResource | null> {
  const client = new MongoClient(MONGO_URL);
...
export async function updateOnlineMatch(
  onlineMatchResource: OnlineMatchResource,
  username?: string
): Promise<boolean> {
  const client = new MongoClient(MONGO_URL);
  let result: boolean = false;
  ...
  try {
    ...
    let room = await privateCollection.findOne({
      roomId: onlineMatchResource.roomId,
    });
    let deleted = false;
    // Search in private rooms
    if (!room) {
      // Search in public rooms. If found and privacy type is now private, delete it from public
      room = await publicCollection.findOne({
        roomId: onlineMatchResource.roomId,
      });
      if (room && onlineMatchResource.privateMatch) {
        await publicCollection.findOneAndDelete({
          roomId: onlineMatchResource.roomId,
        });
        deleted = true;
      }
      ...
      await privateCollection.findOneAndDelete({
        roomId: onlineMatchResource.roomId,
      });
      deleted = true;
    }
...
export async function deleteOnlineMatch(roomId: string): Promise<boolean> {
  ...
  try {
    ...
    // Search for match
    const privateMatch = await client
      .db("OceanCombat")
      .collection("PrivateOnlinematches")
      .findOne({ roomId: roomId });
    const publicMatch = await client
      .db("OceanCombat")
      .collection("PublicOnlinematches")
      .findOne({ roomId: roomId });
    ...
    const db = client.db("OceanCombat");
        ...
        if (gameMode === "1vs1") {
            ...
            if (matches.length === 0) {
                ...
                // console.log("No 1vs1 matches online!");
            ...
            if (matches.length === 0) {
                ...
                // console.log("No Team matches online!");
                return onlineMatches;
            }
            onlineMatches = matches.map((match: any) => ({
                roomId: match.roomId,
            ...
            if (matches.length === 0) {
                ...
                // console.log("No FFA matches online!");
                return onlineMatches;
            }
            onlineMatches = matches.map((match: any) => ({
                roomId: match.roomId,
