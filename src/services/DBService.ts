...
import { OnlineMatchResource, UserResource, calculateLevel } from "../Resources";;
 ...
 */
export async function getUserById(id: string): Promise<UserResource | null> {
    const client = new MongoClient(MONGO_URL);
    ...
    try {
        ...
        const db = client.db("OceanCombat");
        const users = db.collection("Users");
        const user = await users.findOne({ _id: new ObjectId(id) });    // Convert string into ObjectId
        if (user !== null) {
            result = {
...
export async function updateOnlineMatch(onlineMatchResource: OnlineMatchResource, username?: string): Promise<OnlineMatchResource | null> {
    const client = new MongoClient(MONGO_URL);
    ...
    try {
        ...
        // Copy new match data
        const newMatchData: OnlineMatchResource = {
            ...onlineMatchResource
        };
        ...
        // Update match in db
        const result = await collection.findOneAndUpdate({ roomId: onlineMatchResource.roomId }, { $set: newMatchData });
...
export async function getPublicOnlinematches(
  gameMode: string
): Promise<OnlineMatchResource[]> {
  const client = new MongoClient(MONGO_URL);
 ...
 * If user couldn't be found an error is thrown.
 */
export async function activateUserAccount(email: string): Promise<boolean> {
    const client = new MongoClient(MONGO_URL);
    let result: boolean = false;
    ...
    try {
        ...
        if (user === null) {
            ...
            await users.findOneAndUpdate({ _id: user?._id }, { $set: { active: true } });
            result = true;
        }
    } catch (error) {
        throw error;
...
export async function getAllUsers(): Promise<UserResource[]> {
  ...
  let users: UserResource[] = [];
    ...
    try {
        ...
        const userArray = await usersCursor.toArray();
