...
import { BoardTest } from "./gamelogic/BoardTest";
...
export function startWebSocketConnection(server: any) {
  ...
  let playerBoards: { [username: string]: BoardTest } = {};
  io.on("connection", (socket: any) => {
    ...
    socket.on(
      ...
      (
        ...
        const playerBoard = new BoardTest(
          rows,
          cols,
          body.length,
          username,
    ...
    function allPlayersReady(
      io: Server,
      roomId: string,
      playerBoards: { [username: string]: BoardTest },
      gameMode: string,
      maxPlayers: number,
      ...
      if (maxPlayers === 2) {
        // 1vs1 Mode
        const boardsWithRoomId: BoardTest[] = Object.values(playerBoards).filter(
          board => board.roomId === roomId
        );
        ...
        const team1Boards: BoardTest[] = [];
        const team2Boards: BoardTest[] = [];
        ...
        const combinedTeam1Ships: Ship[] = team1Boards.flatMap(board => board.shipArray);
        const combinedTeam2Ships: Ship[] = team2Boards.flatMap(board => board.shipArray);
        const combinedTeam1Mines: Position[] = team1Boards.flatMap(board => board.mines);
        const combinedTeam2Mines: Position[] = team2Boards.flatMap(board => board.mines);
    
        const team1Board = new BoardTest(
          team1Boards[0].rows,
          team1Boards[0].cols,
          combinedTeam1Ships.length,
          Team1Name,
          combinedTeam1Ships.map(ship => ({
        ...
        const team2Board = new BoardTest(
          team2Boards[0].rows,
          team2Boards[0].cols,
          combinedTeam2Ships.length,
          Team2Name,
          combinedTeam2Ships.map(ship => ({
            identifier: ship.identifier,
            direction: ship.isHorizontal ? "X" : "Y",
            startX: ship.startPosition.x,
            startY: ship.startPosition.y,
