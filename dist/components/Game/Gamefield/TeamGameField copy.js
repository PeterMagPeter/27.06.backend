...
exports.default = TeamGameFieldCopy;
const TeamGameFieldCopy_module_css_1 = __importDefault(require("./TeamGameFieldCopy.module.css"));
...
let hiddenLayout = Array.from({ length: rows }, () => Array(cols).fill(null));
...
function TeamGameFieldCopy() {
    ...
    (0, react_1.useEffect)(() => {
        ...
        if (socketInstance_1.default) {
            ...
            socketInstance_1.default.on("shotsReady", (playersReady) => {
            });
    ...
    const changeColor = (button, event) => {
        if (button) {
            ...
            if (event === "Hit") {
                ...
                button.classList.add(TeamGameFieldCopy_module_css_1.default.Hit);
                playSFXSound(hitSound);
            }
            else if (event === "Miss") {
                // button.innerHTML = "O";
                button.setAttribute("data-state", "O");
                button.classList.add(TeamGameFieldCopy_module_css_1.default.Miss);
                playSFXSound(missSound);
            }
            else if (event === "Destroyed") {
                // button.innerHTML = "";
                button.setAttribute("data-state", "D");
                button.classList.add(TeamGameFieldCopy_module_css_1.default.Destroyed);
    ...
    const generateShips = (ships) => {
        ...
        for (let i = 0; i < ships.length; i++) {
            ...
            divs.push(<div style={{
                ...
                }} className={TeamGameFieldCopy_module_css_1.default.miniContainer} key={i}>
          <react_bootstrap_1.Image className={TeamGameFieldCopy_module_css_1.default.miniImagesW} src={shipImage}/>
        </div>);
        }
        return divs;
    };
    ...
    return (<>
      <div className={TeamGameFieldCopy_module_css_1.default.container}>
        <react_bootstrap_1.Image className={TeamGameFieldCopy_module_css_1.default.Logo} src={cof_logo_png_1.default}/>
        <div className={TeamGameFieldCopy_module_css_1.default.whosTurn}>
          {" "}
          <a className={TeamGameFieldCopy_module_css_1.default.playerName}>{enabledPlayer.current}</a>'s TURN
        </div>
        {/* GAME FIELD WHERE PLAYER IS PLAYING */}
        <div className={whichTurn ? TeamGameFieldCopy_module_css_1.default.GameField : TeamGameFieldCopy_module_css_1.default.Ownboard}>
          ...
          {Array.from({ length: fieldSize }, (_, index) => {
            ...
            return (<div className={TeamGameFieldCopy_module_css_1.default[letter2]} style={{ gridArea: letter2 }}>
                <p>{letter}</p>
              </div>);
        })}
          {Array.from({ length: fieldSize }, (_, index) => {
            ...
            let s = whichTurn ? "" : "E";
            let num2 = "NO" + num + s;
            return (<div className={TeamGameFieldCopy_module_css_1.default[num2]} style={{ gridArea: num2 }}>
                <p>{index + 1}</p>
              </div>);
        })}
          {hiddenLayout.map((row, rowIndex) => {
            ...
            return row.map((item, itemIndex) => {
                ...
                const button = document.getElementById(id);
                let newButton = TeamGameFieldCopy_module_css_1.default.button;
                if (button) {
                    newButton =
                        button.getAttribute("data-state") === "X"
                            ? TeamGameFieldCopy_module_css_1.default.Hit
                            : button.getAttribute("data-state") === "D"
                                ? TeamGameFieldCopy_module_css_1.default.Destroyed
                                : button.getAttribute("data-state") === "O"
                                    ? TeamGameFieldCopy_module_css_1.default.Miss
                                    : TeamGameFieldCopy_module_css_1.default.button;
                }
                return (<div className={TeamGameFieldCopy_module_css_1.default[coordinate]} style={{ gridArea: coordinate }}>
                    ...
                    newButton =
                        button.getAttribute("data-state") === "X"
                            ...
                            : button.getAttribute("data-state") === "D"
                                ...
                                : button.getAttribute("data-state") === "O"
                                    ? TeamGameFieldCopy_module_css_1.default.Miss
                                    : TeamGameFieldCopy_module_css_1.default.buttonE;
                }
                return (<div className={TeamGameFieldCopy_module_css_1.default[coordinate]} style={{ gridArea: coordinate }}>
                  <button data-state="" className={newButton} id={positionToString({
                        x: rowIndex,
        ...
        <react_bootstrap_1.Button className={TeamGameFieldCopy_module_css_1.default.Emotes} variant="secondary">
          {" "}
          Emotes
        ...
        <react_bootstrap_1.Modal show={whosTurn} onHide={() => setWhosTurn(false)} backdrop="static" keyboard={false} centered animation={true} className="blur-background">
          <react_bootstrap_1.Modal.Title className={TeamGameFieldCopy_module_css_1.default.modalText}>
...
//# sourceMappingURL=TeamGameField%20copy.js.map