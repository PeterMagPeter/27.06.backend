...
const [layout, setLayout] = (0, react_1.useState)(Array.from({ length: 100 }, (_, index) => ({
    id: index,
})));
let ships = [5, 4, 3, 3, 2, 2];
let shipPositions;
function getRandomIndex(shipsize, position) {
    let index = 0;
    if (position == "h") {
...
function isValidPosition(index, shipsize, position) {
    let valid = false;
    let multiplier = 1;
    if (position == "v")
        multiplier = 10;
...
//# sourceMappingURL=RandomizeShips.js.map