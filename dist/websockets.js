...
function startWebSocketConnection(server) {
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("sendGetLobbies", (gameMode) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (lobbies)
                io.emit("getLobbies", lobbies); // players, hostname
        }));
        socket.on("sendExistRoom", (roomID, username) => __awaiter(this, void 0, void 0, function* () {
            let lobby = yield (0, DBService_1.joinOnlineMatch)(roomID, username);
            console.log(lobby, " roomExists???");
            if (lobby !== null)
                socket.emit("roomExists", roomID);
            else
                socket.emit("NietRoom", roomID);
        }));
        // join a room/ lobby,
        ...
        socket.on("sendPartnerChangedShips", (roomId, ships, username, team, rotated) => {
            // WRITE INTO DB
